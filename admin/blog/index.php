...
// HEAD
$head = new AdminHead();
$head->title('Blog Posts | Site Administration - DuBrowgn.com');
...
$query = "SELECT id, title, date FROM blog ORDER BY id DESC";
if ($stmt = $mysqli->prepare($query)) {
	$stmt->execute();

	echo "{$const_tabs}<table style='min-width:50%'>\n";
	echo "{$const_tabs}\t<tr style='text-decoration:underline'><td /><td>Blog ID</td><td>Title</td><td>Date</td></tr>\n";
...
echo "{$const_tabs}<p>\n";
echo "{$const_tabs}\t<a href='add.php'>Add Blog Post</a>\n";
echo "{$const_tabs}</p>\n";
$cb->close();
